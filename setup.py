...
def run_cmake(arg=""):
    ...
    if ds.find_executable('cmake') is None:
        print "CMake  is required to build krabbe"
        print "Please install cmake version >= 2.8 and re-run setup"
        sys.exit(-1)

    print "Configuring krabbe build with CMake.... "
    cmake_args = arg
    try:
...
setup(
    ...
    install_requires = ["msgpack-python", "PyOpenSSL", "hy", "simplejson", "pyzmq", "ipaddr", "pyfiglet",
                "beanstalkc", "humanfriendly"],
    requires = [],
    include_package_data = True,
    url = 'https://github.com/vulogov/krabbe/',
